...
def _parse_inference_type(value, flag):
  ...
  if value in ["UINT8", "QUANTIZED_UINT8"]:
    return dtypes.uint8
  raise ValueError(
      f"Unsupported value for `{flag}` flag. Expected FLOAT, INT8, UINT8, or QUANTIZED_UINT8 instead got {value}."
  )
...
class _ParseBooleanFlag(argparse.Action):
  ...
  def __call__(self, parser, namespace, values, option_string=None):
    if values is None:
      ...
      raise ValueError(
          f"Invalid argument to --{self.dest}. Must use flag alone, or specify true/false."
      )
...
def _convert_tf1_model(flags):
  ...
  if flags.experimental_select_user_tf_ops:
    ...
    user_op_set = set(
        six.ensure_str(flags.experimental_select_user_tf_ops).split(","))
